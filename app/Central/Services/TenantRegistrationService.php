...
class TenantRegistrationService
{
    ...
    public function processCentralLogin(string $input): CentralLoginResultData
    {
        ...
        if (filter_var($input, FILTER_VALIDATE_EMAIL)) {
            ...
            if ($registrations->isEmpty())
                throw new DomainException('Email ini belum terdaftar atau toko belum selesai disiapkan.', 404);
        ...
        if (TenantRegistration::where('tenant_id', $slug)->exists())
            throw new DomainException('Toko Anda sedang disiapkan atau menunggu pembayaran. Silakan cek status pendaftaran.', 403);

        throw new DomainException('Toko atau Email tidak ditemukan. Harap periksa kembali.', 404);
    }
    ...
    public function requestOtp(string $email): void
    {
        ...
        if (Cache::get('email_verified_' . $email))
            throw new DomainException('Email ini sudah terverifikasi.', 400);
        ...
        try {
            ...
            throw new DomainException('Gagal mengirim email OTP. Silakan coba lagi.', 500);
        }
    }
    ...
    public function verifyOtp(string $email, string $otp): void
    {
        ...
        if (!$cachedOtp || $cachedOtp !== $otp)
            throw new DomainException('Kode OTP tidak valid atau sudah kedaluwarsa.', 400);
    ...
    public function initiateRegistration(array $data, string $ip, bool $hasFreeTrialCookie): RegistrationResultData
    {
        ...
        if (Tenant::where('id', $slug)->exists() || TenantRegistration::where('tenant_id', $slug)->where('status', 'paid')->exists())
            throw new DomainException('Nama toko (subdomain) ini sudah terpakai. Silakan gunakan nama lain.');
        ...
        if ($data['payment_method'] === 'manual') {
            ...
            $this->sendBillingEmail($registration, 'manual', ['wa_url' => $waUrl]);
        ...
        if ($data['payment_method'] === 'midtrans') {
            ...
            $this->sendBillingEmail($registration, 'midtrans');
        ...
        $this->sendBillingEmail($registration, 'duitku', ['payment_url' => $duitkuInvoice->paymentUrl]);
    ...
    public function completeRegistration(TenantRegistration $registration): void
    {
        try {
            ...
            $this->handleRegistrationFailure($registration, $e, $emailBody, '[TenantRegistrationService] Failed to create tenant after payment');
        }
    }
    ...
    public function completeFreePlanRegistration(TenantRegistration $registration): array
    {
        try {
            ...
            $this->handleRegistrationFailure($registration, $e, $emailBody, '[TenantRegistrationService] Free registration failed');
