...
class AuthController extends Controller
{
    ...
    public function apiRegisterStatus($invoiceCode)
    {
        ...
        if (!$registration) {
            return $this->failResponse(errors: [], code: ResponseAlias::HTTP_NOT_FOUND, message: 'Registration not found');
        }
    ...
    public function centralLogin(Request $request)
    {
        ...
        if (filter_var($input, FILTER_VALIDATE_EMAIL)) {
            ...
            if ($registrations->isEmpty()) {
                return $this->failResponse(errors: [], code: ResponseAlias::HTTP_NOT_FOUND, message: 'Email ini belum terdaftar atau toko belum selesai disiapkan.');
            }
        ...
        if ($regPending) {
            return $this->failResponse(errors: [], code: ResponseAlias::HTTP_FORBIDDEN, message: 'Toko Anda sedang disiapkan atau menunggu pembayaran. Silakan cek status pendaftaran.');
        }

        return $this->failResponse(errors: [], code: ResponseAlias::HTTP_NOT_FOUND, message: 'Toko atau Email tidak ditemukan. Harap periksa kembali.');
    }
    ...
    public function requestOtp(Request $request)
    {
        ...
        if (Cache::get('email_verified_' . $email)) {
            return $this->failResponse(errors: [], code: ResponseAlias::HTTP_BAD_REQUEST, message: 'Email ini sudah terverifikasi.');
        }
        ...
        try {
            ...
            return $this->successResponse(data: [], message: 'OTP berhasil dikirim ke email Anda.');
        } catch (Exception $e) {
            Log::error("Failed to send OTP email: " . $e->getMessage());
            return $this->errorResponse(errors: [], message: 'Gagal mengirim email OTP. Silakan coba lagi.', code: ResponseAlias::HTTP_INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public function verifyOtp(Request $request)
    {
        ...
        if (!$cachedOtp || $cachedOtp !== $request->otp) {
            return $this->failResponse(errors: [], code: ResponseAlias::HTTP_BAD_REQUEST, message: 'Kode OTP tidak valid atau sudah kedaluwarsa.');
        }
        ...
        return $this->successResponse(data: [], message: 'Email berhasil diverifikasi!');
    }
    ...
    public function registerTenant(Request $request)
    {
        ...
        if (!Cache::get('email_verified_' . $validated['email'])) {
            return $this->failResponse(errors: [], code: ResponseAlias::HTTP_BAD_REQUEST, message: 'Email belum diverifikasi. Silakan verifikasi email terlebih dahulu.');
        }
        ...
        if ($validated['paket'] === 'free') {
            ...
            if (RateLimiter::tooManyAttempts($rateKey, 2)) {
                ...
                return $this->failResponse(errors: [], code: ResponseAlias::HTTP_TOO_MANY_REQUESTS, message: 'Batas pendaftaran toko gratis terlampaui untuk perangkat Anda. Silakan coba lagi dalam ' . ceil($seconds / 60) . ' menit.');
            }

            // Layer 2: Cookie Fingerprint Protection
            if ($request->hasCookie('pakaiapp_free_trial_claimed')) {
                return $this->failResponse(errors: [], code: ResponseAlias::HTTP_FORBIDDEN, message: 'Perangkat/Browser Anda terdeteksi sudah pernah mendaftarkan Toko Gratis. Untuk mendaftarkan toko tambahan, silakan pilih Paket Santai atau Paket Premium.');
            }
            ...
            if (in_array($emailDomain, self::DISPOSABLE_EMAIL_DOMAINS)) {
                return $this->failResponse(errors: [], code: ResponseAlias::HTTP_BAD_REQUEST, message: 'Pendaftaran Toko Gratis dibatasi. Silakan gunakan alamat email utama/resmi Anda (seperti Gmail, Yahoo, Outlook, atau domain instansi).');
            }
            ...
            if ($hasFreeStore) {
                return $this->failResponse(errors: [], code: ResponseAlias::HTTP_BAD_REQUEST, message: 'Alamat email ini sudah terdaftar untuk Toko Gratis lainnya. Untuk mendaftarkan toko tambahan, silakan pilih Paket Santai atau Paket Premium.');
            }
            ...
            if ($hasFreeWa) {
                return $this->failResponse(errors: [], code: ResponseAlias::HTTP_BAD_REQUEST, message: 'Nomor WhatsApp ini sudah terdaftar untuk Toko Gratis lainnya. Silakan gunakan nomor lain atau pilih Paket Santai/Premium.');
            }
        ...
        if (Tenant::where('id', $slug)->exists() || TenantRegistration::where('tenant_id', $slug)->where('status', 'paid')->exists()) {
            return $this->failResponse(errors: [], code: ResponseAlias::HTTP_BAD_REQUEST, message: 'Nama toko (subdomain) ini sudah terpakai. Silakan gunakan nama lain.');
        }
        ...
        if ($amount == 0) {
            ...
            try {
                ...
                ], message: 'Toko berhasil dibuat! Anda akan dialihkan ke dashboard.')->withCookie($cookie);
            } catch (Exception $e) {
                ...
                return $this->errorResponse(errors: [], message: 'Terjadi kesalahan sistem saat membuat toko.', code: ResponseAlias::HTTP_INTERNAL_SERVER_ERROR);
            }
        }
        ...
        if ($validated['payment_method'] === 'midtrans') {
            try {
                ...
                return $this->errorResponse(errors: [], message: 'Gagal terhubung ke layanan pembayaran Midtrans.', code: ResponseAlias::HTTP_INTERNAL_SERVER_ERROR);
            }
        }
        ...
        try {
            ...
            return $this->errorResponse(errors: [], message: 'Gagal terhubung ke layanan pembayaran Duitku.', code: ResponseAlias::HTTP_INTERNAL_SERVER_ERROR);
        }
    }
