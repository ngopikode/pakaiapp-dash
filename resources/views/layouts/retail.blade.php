@php
    ...
    if ($setting) {
        ...
        $storeName = $setting->name ?: 'EzRetail';

        $orderTypes = [];
        if ($setting->is_takeaway_active) $orderTypes[] = ['id' => 'takeaway', 'label' => 'Ambil Sendiri'];
        if ($setting->is_delivery_active) $orderTypes[] = ['id' => 'delivery', 'label' => 'Pengiriman'];
        if (empty($orderTypes))           $orderTypes[] = ['id' => 'delivery', 'label' => 'Dikirim'];
    }
@endphp
...
@else
    ...
    <head>
        ...
        <title>{{ $setting->seo_title ?: ($storeName . ' | Toko Online') }}</title>
        <meta name="description"
              content="{{ $setting->seo_description ?: 'Platform toko online modern.' }}"/>
        <meta name="keywords" content="{{ $setting->seo_keywords ?: 'toko online, retail, belanja' }}"/>
        ...
        <meta property="og:description"
              content="{{ $setting->og_description ?: 'Belanja produk terbaik dari kami.' }}"/>
        @if($setting->og_image)
    ...
    {{-- ===== PAGE LOADER ===== --}}
    @include('pages.tenant.retail.modals._loader')
    ...
    <div
        ...
        x-data="storeApp"
        data-default-order-type="{{ collect($orderTypes)->first()['id'] ?? 'delivery' }}"
    ...
    >
        {{-- ===== NAVBAR ===== --}}
        @include('components.layouts.retail._navbar', ['setting' => $setting])

        {{-- ===== HERO ===== --}}
        @include('pages.tenant.retail._hero', ['setting' => $setting])
        ...
        >
            <div @click.stop class="bg-white w-full max-w-xs rounded-2xl p-8 text-center relative overflow-hidden">
                ...
                <div class="relative z-10">
                    ...
                    <h2 class="text-xl font-black text-zinc-900 mb-1">SCAN TOKO</h2>
                    ...
                    <div class="bg-white p-2 rounded-xl border-2 border-dashed border-zinc-200 mb-6">
                        <img :src="qrUrl" alt="QR Code Toko" class="w-full aspect-square rounded-lg opacity-90"/>
                    </div>
        ...
        {{-- ===== OPTION MODAL ===== --}}
        @include('pages.tenant.retail.modals.option-modal')

        {{-- ===== CHECKOUT MODAL ===== --}}
        @include('pages.tenant.retail.modals.checkout-modal', ['orderTypes' => $orderTypes])

        {{-- ===== RIWAYAT MODAL ===== --}}
        @include('pages.tenant.retail.modals.history-modal')
    </div>
