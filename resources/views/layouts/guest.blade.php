...
<body class="antialiased font-sans text-gray-900 bg-white dark:bg-gray-950 selection:bg-brand-accent/30 transition-colors duration-300">
    <div class="min-h-screen flex w-full">
        ...
        @endphp
        <div class="hidden lg:flex lg:w-1/2 relative bg-[#090D16] overflow-hidden items-center justify-center border-r border-gray-200/5 dark:border-gray-800/10">
            ...
            <div class="relative z-10 p-16 xl:p-24 max-w-2xl flex flex-col justify-between h-full w-full">
                ...
                <div class="my-auto space-y-6">
                    ...
                    <p class="text-slate-400 text-sm xl:text-base leading-relaxed max-w-sm mt-6">
                        Kelola kasir, pantau performa penjualan, dan kembangkan bisnis dengan teknologi terdepan dalam satu genggaman.
                    </p>
                </div>
        ...
        <div class="w-full lg:w-1/2 flex flex-col items-center justify-center p-6 sm:p-12 relative bg-white dark:bg-gray-950">
            ...
            <div class="lg:hidden absolute top-6 left-6 right-6 flex items-center gap-3">
                 @if($storeSettings && $storeSettings->logo)
                     <img src="{{ Storage::url($storeSettings->logo) }}" alt="{{ $storeSettings->name }}" class="w-8 h-8 rounded-lg shadow-sm border border-gray-100 dark:border-gray-800">
                     <span class="text-xl font-bold font-heading text-gray-900 dark:text-white tracking-tight">{{ $storeSettings->name }}</span>
                 @else
                     <img src="/android-chrome-192x192.png" alt="Logo" class="w-8 h-8 rounded-lg shadow-sm border border-gray-100 dark:border-gray-800">
                     <span class="text-xl font-bold font-heading text-gray-900 dark:text-white tracking-tight">pakaiapp</span>
                 @endif
            </div>

            <!-- Main Content Slot (Form blends into background, no card) -->
            <div class="w-full max-w-[400px] relative z-10 mt-12 lg:mt-0 animate-in fade-in slide-in-from-bottom-4 duration-700">
                {{ $slot }}
            </div>
