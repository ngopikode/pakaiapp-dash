...
new class extends Component {
    ...
    public string $title = 'EzMenu';
    public string $subtitle = 'Menu Digital';
    public string $logo = '';
    public string $themeColor = '#f59e0b';

    public function mount()
    {
        $store = StoreSetting::first();
        if ($store) {
            $this->brandText = $store->navbar_brand_text ?? 'Ez';
            $this->title = $store->navbar_title ?? $store->name ?? 'EzMenu';
            $this->subtitle = $store->navbar_subtitle ?? 'Menu Digital';
            $this->logo = $store->logo ?? '';
            $this->themeColor = $store->theme_color ?? '#f59e0b';
        }
    }
};
?>
    ...
    <div class="max-w-xl mx-auto flex justify-between items-center px-5 py-3">
        <div class="flex items-center gap-3">
            <div class="bg-gradient-to-br from-[var(--primary-color)] to-[var(--primary-color)] w-10 h-10 rounded-xl flex items-center justify-center shadow-lg shadow-[var(--primary-color)]/20 border border-black/5 rotate-[-2deg] hover:rotate-0 transition-transform duration-300 overflow-hidden shrink-0">
                @if($logo)
                    <img src="{{ Storage::url($logo) }}" alt="{{ $title }}" class="w-full h-full object-cover" />
                @else
