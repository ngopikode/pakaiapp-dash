...
<div class="fixed-bottom d-md-none pb-safe bottom-navbar-mobile" style="z-index: 1040;" wire:poll.15s.visible>
    <div class="bg-white border-top shadow-lg p-0" style="border-radius: 1.5rem 1.5rem 0 0; padding: 0.25rem;">
        <div class="d-flex justify-content-around align-items-center" style="height: 60px;">
            @foreach($this->navItems as $item)
                ...
                <a href="{{ route($item['route']) }}" wire:navigate onclick="window.showLoader()"
                   class="text-decoration-none d-flex flex-column align-items-center justify-content-center w-100 h-100 position-relative p-0 {{ $isActive ? 'text-primary' : 'text-secondary' }}">
                    <div class="position-relative">
                        ...
                        @if($item['route'] === 'order' && $this->pendingOrdersCount > 0)
                            <span
                                ...
                                style="font-size: 0.55rem; width: 18px; height: 18px;">
                    ...
                    @if($isActive)
                        <div class="position-absolute top-0 start-50 translate-middle-x bg-primary rounded-bottom"
                             style="width: 20px; height: 3px;"></div>
                    @endif
                </a>
            @endforeach
            ...
            <!-- Lainnya Toggle -->
            <button
                class="btn border-0 text-decoration-none d-flex flex-column align-items-center justify-content-center w-100 h-100 position-relative text-secondary p-0"
