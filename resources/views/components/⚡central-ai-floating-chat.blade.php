...
<div>
    <div class="central-chat-wrapper"
        ...
        <div x-show="isOpen"
                ...
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div class="lh-sm">
                        ...
                        <span class="text-white-50"
                              style="font-size: 11px; letter-spacing: 0.5px;">Selalu siap membantu</span>
                    </div>
                </div>
                <button type="button" class="btn btn-link text-white-50 p-1 m-0 text-decoration-none"
            ...
            <div class="flex-grow-1 overflow-y-auto p-3"
                ...
                <div class="text-center mb-4 mt-1">
                    <span class="badge rounded-pill px-3 py-2 text-uppercase"
                          style="font-size: 10px; letter-spacing: 1px; color: var(--chat-text); opacity: 0.7; background-color: var(--chat-badge-bg);">Ngobrol dengan AI Kami</span>
                </div>
                ...
                @foreach($messages as $msg)
                    @if($msg['role'] !== 'system')
                        <div
                            ...
                            <div
                                class="p-3 shadow-sm {{ $msg['role'] === 'user' ? 'user-msg-bubble' : 'bot-msg-bubble markdown-content' }}"
            ...
            <!-- Input Area -->
            <div class="border-top p-3 flex-shrink-0 position-relative"
                 style="background-color: var(--chat-input-area-bg); border-color: var(--chat-border) !important;">
                ...
                <div class="text-center mb-2" style="padding-right: 52px;">
                    <a href="https://wa.me/6285172441544" target="_blank" rel="noopener noreferrer"
                       class="btn btn-sm btn-outline-success rounded-pill px-3 py-1.5 fw-bold d-inline-flex align-items-center justify-content-center gap-2 w-100"
                       style="font-size: 12px; border-width: 1.5px;">
                ...
                <form wire:submit="sendMessage" class="d-flex align-items-center gap-2 m-0">
                    <input type="text" class="form-control rounded-pill px-4"
                           style="font-size: 14px; padding-top: 0.65rem; padding-bottom: 0.65rem; background-color: var(--chat-input-bg); color: var(--chat-input-text); border: none;"
                           wire:model="userInput" placeholder="Tanya sesuatu..." autocomplete="off" required>
                    <button type="submit"
                            class="btn central-chat-send-btn rounded-circle d-flex align-items-center justify-content-center flex-shrink-0"
                            style="width: 44px; height: 44px;" wire:loading.attr="disabled">
                        <div wire:loading.remove wire:target="sendMessage">
                            <i class="bi bi-send-fill"></i>
                        </div>
                        <div wire:loading wire:target="sendMessage" class="spinner-border spinner-border-sm text-white"
                             role="status"></div>
                    </button>
                </form>
        ...
        <!-- Floating Button & CTA -->
        <div class="d-flex justify-content-end align-items-center gap-3 position-relative"
             :class="isOpen && isMobile ? 'd-none' : ''">
            <!-- CTA Tooltip -->
            <div x-show="!isOpen && showTooltip"
                ...
                <button class="btn btn-link p-0 m-0 ms-1 text-decoration-none d-flex align-items-center"
                        @click.stop="showTooltip = false" style="line-height: 1; color: inherit; opacity: 0.7;">
            ...
            <button type="button"
                    class="btn btn-dark rounded-circle shadow-lg d-flex align-items-center justify-content-center border border-2 border-white"
                    style="width: 56px; height: 56px; z-index: 10;" @click="isOpen = !isOpen; showTooltip = false">
