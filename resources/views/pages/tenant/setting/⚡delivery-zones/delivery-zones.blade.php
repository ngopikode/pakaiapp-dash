<div class="relative w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 mt-6 sm:mt-10 font-sans pb-28"
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 pb-6 border-b border-slate-200 dark:border-slate-800">
        <div>
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-2">Daftar wilayah pengiriman, biaya ongkir, dan gratis ongkir.</p>
        </div>
                ...
                @island(name: 'list', defer: true)
                    ...
                    @forelse($this->zones as $zone)
                        <div class="p-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl flex items-start justify-between gap-3 group hover:border-orange-500 transition-colors cursor-pointer mb-3"
                            ...
                            <div class="flex flex-col items-center gap-1 shrink-0 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                                <button @click.stop="editZone({{ $zone->toJson() }})" title="Edit" class="p-1.5 text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-500/10 rounded-lg transition-colors">
                                    <i class="ph-bold ph-pencil-simple text-lg"></i>
                                </button>
                                <button @click.stop="confirmDelete({{ $zone->id }})" title="Hapus" class="p-1.5 text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg transition-colors">
                    ...
                    @empty
                        <div class="text-center py-10 px-4">
                            ...
                            <p class="text-xs text-slate-500 mt-1">Tambahkan zona pengiriman agar pembeli bisa memilih area saat checkout.</p>
                        </div>
                    @endforelse
                @endisland
            </div>
        ...
        <div class="md:w-7/12 lg:w-2/3 flex flex-col bg-white dark:bg-slate-900 min-h-0
            ...
            <div class="flex-1 overflow-y-auto p-6 md:p-8 space-y-6">
                <div class="flex justify-between items-start">
                    <div>
                        <h3 class="text-lg font-black text-slate-800 dark:text-white mb-1" x-text="form.id ? 'Edit Zona' : 'Tambah Zona Baru'"></h3>
                        <p class="text-sm text-slate-500 dark:text-slate-400">Atur nama wilayah dan tarif ongkos kirim.</p>
                    </div>
                ...
                <div class="space-y-5">
                    <div>
                        <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Nama Area/Zona <span class="text-red-500">*</span></label>
                        <input type="text" x-model="form.name" placeholder="Contoh: Pusat Kota, Luar Kota, Sri Gunting" class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-sm font-semibold rounded-xl px-4 py-3 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-all outline-none text-slate-800 dark:text-white">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Tarif Ongkos Kirim (Rp) <span class="text-red-500">*</span></label>
                            <input type="number" x-model.number="form.shipping_cost" placeholder="0" class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-sm font-semibold rounded-xl px-4 py-3 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-all outline-none text-slate-800 dark:text-white">
                            <span x-show="errors.shipping_cost" class="text-[11px] text-red-500 mt-1.5 block font-bold" x-text="errors.shipping_cost" style="display:none;"></span>
                        </div>

                        <div>
                            <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Min. Belanja Gratis Ongkir (Rp)</label>
                            <input type="number" x-model.number="form.min_free_shipping" placeholder="0" class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-sm font-semibold rounded-xl px-4 py-3 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-all outline-none text-slate-800 dark:text-white">
                            <p class="text-[11px] text-slate-500 mt-2 leading-relaxed">Isi dengan 0 jika area ini tidak memiliki promo gratis ongkir.</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl">
                        <div>
                            ...
                            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Tampilkan zona ini di pilihan checkout pembeli.</p>
                        </div>
            ...
            <div class="p-6 border-t border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 shrink-0 pb-10 md:pb-6">
                <button @click="saveZone()" :disabled="isSaving" class="w-full py-3.5 bg-orange-500 hover:bg-orange-600 text-white text-sm font-black rounded-xl shadow-lg shadow-orange-500/25 active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50">
                    <span x-show="!isSaving">Simpan Pengaturan Zona</span>
