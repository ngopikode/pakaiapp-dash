...
<div class="relative flex flex-col overflow-hidden rounded-2xl border {{ $cardBorder }} bg-white dark:bg-slate-900 shadow-sm border-slate-200 dark:border-slate-800 transition-all">
    ...
    <div class="flex items-center justify-between p-4 {{ $headerBg }} {{ $headerText }}">
        <div>
            <h5 class="text-2xl font-black">#{{ str_replace('INV-', '', $order->invoice_code) }}{{ $isWaiting && $order->items->where('kitchen_status', '!=', 'waiting')->isNotEmpty() ? ' (Tambahan)' : '' }}</h5>
            <small class="mt-1 flex items-center gap-1 font-bold {{ $isReady ? 'text-emerald-100' : ($isProcessing ? 'text-amber-900/80' : 'text-red-100') }}"
                   x-data="{
                       timeAgo: '',
        ...
        <div class="text-right">
            @if($order->order_type === 'dinein')
                <span class="inline-flex items-center gap-1 rounded-full {{ $badgeBg }} px-3 py-1.5 text-xs font-bold {{ $badgeText }}">
