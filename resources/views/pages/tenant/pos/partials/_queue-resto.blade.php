@if($pendingOrders->isEmpty())
    <div class="flex min-h-[40vh] flex-col items-center justify-center rounded-3xl border border-emerald-800/15 bg-white/80 py-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div class="mb-4 flex h-24 w-24 items-center justify-center rounded-full bg-slate-100 shadow-sm dark:bg-slate-800">
            <i class="bi bi-receipt text-4xl text-slate-400 dark:text-slate-600"></i>
        </div>
            ...
            <div class="queue-card flex h-full flex-col overflow-hidden rounded-2xl">
                ...
                <div class="flex items-start justify-between p-4 queue-header">
                    ...
                    <div class="flex flex-col items-end gap-2">
                        <span class="inline-flex items-center rounded-full bg-amber-100 px-3 py-1 text-[11px] font-extrabold uppercase tracking-wider text-amber-800 shadow-sm dark:bg-amber-500/20 dark:text-amber-400">
                            <i class="bi bi-hourglass-split me-1"></i>PENDING
                        </span>
                        @if($order->is_online)
                            <span class="inline-flex items-center rounded-full border border-emerald-800/20 bg-emerald-50 px-3 py-1 text-[11px] font-extrabold text-emerald-800 shadow-sm dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-400">
                ...
                <div class="flex flex-1 flex-col">
                    ...
                    <div class="flex flex-wrap gap-2 px-4 py-3 queue-meta-area">
                        <span class="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-700 shadow-sm dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200">
                            <i class="bi bi-person text-slate-400"></i> {{ $order->customer_name }}
                        </span>
                        @if($order->table_number)
                            <span class="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-700 shadow-sm dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200">
                                <i class="bi bi-hash text-slate-400"></i> Meja {{ $order->table_number }}
                            </span>
                        @endif
                        <span class="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-bold capitalize text-slate-700 shadow-sm dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200">
                            <i class="bi bi-bag text-slate-400"></i> {{ $order->order_type }}
                        </span>
                        @if($order->notes)
                            <span class="inline-flex items-center gap-1 rounded-full border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-bold text-amber-700 shadow-sm dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-400" title="Catatan: {{ $order->notes }}">
                    ...
                    <div class="flex-1 px-4 py-3" style="font-size: 0.85rem;">
                            ...
                            <div class="item-row mb-1 flex items-start justify-between rounded-2xl px-3 py-2.5">
                                <div class="pe-2 text-slate-900 dark:text-slate-100">
                                    <div class="flex items-start gap-2">
                                        <span class="inline-flex items-center rounded-lg bg-emerald-800 px-2 py-1 text-[11px] font-extrabold text-white dark:bg-emerald-500 dark:text-slate-950">{{ $item->quantity }}x</span>
                                        <div>
                                            ...
                                            @endif
                                            @if($item->discount > 0)
                                                <div class="mt-1 text-xs font-bold text-emerald-600 dark:text-emerald-400"><i class="bi bi-tag-fill me-1"></i>Hemat Rp {{ number_format($item->discount * $item->quantity, 0, ',', '.') }}</div>
                                            @endif
                                            @if($item->note)
                                                <div class="mt-1 text-xs italic text-amber-600 dark:text-amber-400"><i class="bi bi-chat-dots me-1"></i>{{ $item->note }}</div>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="px-2">
                            @if(($order->service_charge_amount ?? 0) > 0)
                                <div class="flex items-center justify-between py-1 text-xs font-semibold text-slate-500 dark:text-slate-400">
                                    <span>Biaya Layanan ({{ number_format($order->service_charge_percentage ?? 5, 0) }}%)</span>
                                    <span class="font-bold text-slate-900 dark:text-white">Rp {{ number_format($order->service_charge_amount, 0, ',', '.') }}</span>
                                </div>
                            @endif
                            @if(($order->tax_amount ?? 0) > 0)
                                <div class="flex items-center justify-between py-1 text-xs font-semibold text-slate-500 dark:text-slate-400">
                                    <span>Pajak PB1 ({{ number_format($order->tax_percentage ?? 10, 0) }}%)</span>
                                    <span class="font-bold text-slate-900 dark:text-white">Rp {{ number_format($order->tax_amount, 0, ',', '.') }}</span>
                                </div>
                            @endif
                            @if(($order->discount ?? 0) > 0)
                                <div class="flex items-center justify-between py-1 text-xs font-bold text-emerald-600 dark:text-emerald-400">
                                    <span><i class="bi bi-scissors me-1"></i>Diskon</span>
                    ...
                    {{-- Total Box --}}
                    <div class="mx-4 mb-4 mt-auto flex items-center justify-between rounded-2xl border p-3 shadow-sm queue-total-box">
                        <span class="text-[11px] font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300">Total Pembayaran</span>
                        <h4 class="mb-0 font-black tracking-tight text-emerald-800 dark:text-emerald-400">Rp {{ number_format($order->total_price ?? $order->subtotal, 0, ',', '.') }}</h4>
                    </div>
                </div>
                ...
                <div class="border-t border-emerald-800/10 bg-white p-3 dark:border-slate-800 dark:bg-slate-900">
                    <div class="mb-3 flex gap-2">
                        ...
                        <button @click="openSplitModal({{ json_encode([
                            ...
                            <i class="bi bi-layout-split text-lg"></i>
                        </button>
                        @endif
                    ...
                    <button @click="openPayForOrder({{ json_encode([
                            ...
                            style="border: none;">
                        <i class="bi bi-cash-coin text-lg"></i> Bayar Sekarang
                    </button>
                </div>
            </div>
        @endforeach
