...
<header class="relative bg-[var(--background)] border-none mb-4">
    ...
    <div class="h-40 w-full relative overflow-hidden bg-[var(--bg-soft)]">
        ...
        @if($ogImage)
            ...
            <img src="{{ $ogImage }}" alt="{{ $storeName }}"
                 ...
                 loading="eager">
        @else
            <div class="absolute inset-0" style="background: linear-gradient(160deg, var(--primary-color) 0%, color-mix(in srgb, var(--primary-color) 55%, #000) 100%);"></div>
        @endif

        {{-- Gradient overlay — stronger to dim the image --}}
        <div class="absolute inset-0 bg-gradient-to-t from-[var(--background)] via-black/40 to-black/30 pointer-events-none"></div>
        ...
        {{-- CENTER: Tagline — serif italic brand statement --}}
        @if($setting->hero_tagline)
            <div class="absolute inset-0 flex items-center justify-center z-10 pointer-events-none px-10">
                <p class="text-sm font-serif italic text-white/90 text-center leading-relaxed drop-shadow max-w-[280px]"
    ...
    <div class="px-5 -mt-10 relative z-10 flex items-center gap-4 pb-4">

        {{-- Logo — float up, left-aligned --}}
        <div class="w-20 h-20 rounded-full overflow-hidden bg-[var(--bg-soft)] ring-2 ring-[var(--border)] shadow-xl shrink-0 relative" style="background-color: var(--surface);">
            @if($logoUrl)
                <img src="{{ $logoUrl }}" alt="{{ $storeName }}" class="w-full h-full object-cover">
            @else
                <div class="w-full h-full flex items-center justify-center" style="background: linear-gradient(135deg, var(--primary-color), color-mix(in srgb, var(--primary-color) 60%, #000));">
                    <span class="text-2xl font-black text-white">{{ substr($storeName, 0, 2) }}</span>
                </div>
            @endif
            {{-- Live dot --}}
            <div class="absolute bottom-1 right-1 w-4 h-4 rounded-full border-2 border-[var(--surface)] shadow-sm {{ $setting->is_active ? 'bg-emerald-500' : 'bg-red-400' }}"></div>
        </div>
            ...
            @if($setting->hero_headline || $setting->navbar_subtitle)
                <div class="flex items-center gap-1.5 flex-wrap mt-0.5">
                    @if($setting->hero_headline)
                        <span class="text-sm text-[var(--text-secondary)]">{{ $setting->hero_headline }}</span>
                    @endif
                    @if($setting->hero_headline && $setting->navbar_subtitle)
                        <span class="w-1 h-1 rounded-full bg-[var(--border)] opacity-60 shrink-0"></span>
                    @endif
                    @if($setting->navbar_subtitle)
                        <span class="text-xs text-[var(--text-secondary)] opacity-70">{{ $setting->navbar_subtitle }}</span>
                    @endif
                </div>
            @endif

            @if($setting->hero_status_text)
                <div class="inline-flex items-center gap-1.5 mt-2 px-2.5 py-1 rounded-xl bg-[var(--bg-soft)] border border-[var(--border)] text-[10px] font-black uppercase tracking-widest text-[var(--text-secondary)]">
                    <div class="w-1.5 h-1.5 rounded-full {{ $setting->is_active ? 'bg-emerald-500 animate-pulse' : 'bg-red-400' }}"></div>
