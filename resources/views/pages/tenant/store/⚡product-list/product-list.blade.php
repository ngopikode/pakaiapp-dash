...
>
    ...
    <div id="menu-start"
         class="scroll-mt-0 sticky top-[57px] z-40 bg-zinc-50/90 backdrop-blur-xl py-2.5 border-b border-zinc-100/50 shadow-sm shadow-zinc-100/50">
        <div class="max-w-xl mx-auto px-5 flex items-center justify-between gap-3">
            ...
            <div class="flex bg-white p-1 rounded-xl border border-zinc-200/80 shadow-sm shrink-0">
                <button
                    @click="viewMode = 'list'"
                    :class="viewMode === 'list' ? 'bg-zinc-900 text-white shadow-sm' : 'text-zinc-300 hover:text-zinc-500'"
                ...
                <button
                    @click="viewMode = 'grid'"
                    :class="viewMode === 'grid' ? 'bg-zinc-900 text-white shadow-sm' : 'text-zinc-300 hover:text-zinc-500'"
                    class="p-2 rounded-lg transition-all duration-200"
        ...
        >
            @forelse($this->products as $index => $item)
                ...
                <div
                    ...
                    :class="[
                        viewMode === 'grid' ? 'flex-col h-full' : 'flex-row items-center gap-3 p-3',
                        !item.is_active ? 'opacity-90' : '',
                        showStepper ? 'border-[var(--primary-color)]/40 ring-2 ring-[var(--primary-color)]/10' : ''
                    ]"
                ...
                >
                    ...
                    >
                        ...
                        @else
                            <div
                                class="w-full h-full bg-gradient-to-br from-zinc-100 to-zinc-200 flex flex-col items-center justify-center gap-2">
                                ...
                                <span
                                    class="text-[9px] font-bold text-zinc-300 uppercase tracking-widest">No Image</span>
                            </div>
                        @endif
                        ...
                        <div class="absolute top-2.5 left-2.5 flex flex-col gap-2 z-20">
                            ...
                            <button
                                ...
                                <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24"
                                     fill="currentColor"
                                     class="text-zinc-700 group-hover/story:text-white transition-colors">
                                    <path
                            ...
                            <template x-if="showStepper">
                                <div class="flex items-center justify-between bg-zinc-900 rounded-xl p-1 shadow-md">
                                    <button @click="updateQty(item.name, -1)"
                                            class="w-8 h-8 flex items-center justify-center text-white hover:bg-zinc-700 rounded-lg transition-colors active:scale-90">
                                    ...
                                    <button @click="updateQty(item.name, 1)"
                                            class="w-8 h-8 flex items-center justify-center text-zinc-900 bg-[var(--primary-color)] rounded-lg transition-all active:scale-90 hover:brightness-110">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14"
                                             viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                             stroke-linecap="round" stroke-linejoin="round">
        ...
        <div class="fixed bottom-6 left-0 right-0 px-5 z-50 animate-slide-up">
            ...
            >
                ...
                <div class="relative flex items-center gap-3.5">
                    <div
                        class="bg-[var(--primary-color)] text-zinc-900 w-11 h-11 rounded-xl flex items-center justify-center font-black text-sm shadow-md shadow-[var(--primary-color)]/25"
                        x-text="totalQty"></div>
                    <div class="text-left">
