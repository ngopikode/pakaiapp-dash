    ...
    x-data="{
        ...
        async handleTouchEnd() {
            ...
            if (this.pullY > 60) {
                ...
                this.pullY = 60;
                await $wire.setCategory($wire.category); // re-fetch by resetting
...
>
    ...
    <div id="menu-start" class="scroll-mt-0 sticky top-[57px] z-40 bg-zinc-50/90 backdrop-blur-xl py-2.5 border-b border-zinc-100/50 shadow-sm shadow-zinc-100/50">
        <div class="max-w-xl mx-auto px-5 flex items-center justify-between gap-3">
            ...
            <div class="flex bg-white p-1 rounded-xl border border-zinc-200/80 shadow-sm shrink-0">
                <button wire:click="setViewMode('list')" class="p-2 rounded-lg transition-all duration-200 {{ $viewMode === 'list' ? 'bg-zinc-900 text-white shadow-sm' : 'text-zinc-300 hover:text-zinc-500' }}">
                ...
                <button wire:click="setViewMode('grid')" class="p-2 rounded-lg transition-all duration-200 {{ $viewMode === 'grid' ? 'bg-zinc-900 text-white shadow-sm' : 'text-zinc-300 hover:text-zinc-500' }}">
    ...
    {{-- Loading Skeleton (shown while Livewire is requesting) --}}
    <div wire:loading.class.remove="hidden" wire:target="setCategory, setViewMode" class="hidden">
        <main class="max-w-xl mx-auto px-5 mt-4 {{ $viewMode === 'grid' ? 'grid grid-cols-2 gap-3' : 'flex flex-col gap-3' }}">
            @for($s = 0; $s < 6; $s++)
                <div class="bg-white rounded-2xl border border-zinc-100/80 shadow-sm flex overflow-hidden {{ $viewMode === 'grid' ? 'flex-col h-full' : 'flex-row items-center gap-4 p-3' }}">
                    {{-- Image Skeleton --}}
                    <div class="bg-zinc-200/70 animate-pulse shrink-0 {{ $viewMode === 'grid' ? 'w-full aspect-[4/3]' : 'w-20 h-20 rounded-xl' }}"></div>
                    {{-- Content Skeleton --}}
                    <div class="flex-1 flex flex-col justify-between min-h-0 {{ $viewMode === 'grid' ? 'p-3.5 pt-2.5' : '' }}">
                        <div>
                            <div class="h-4 bg-zinc-200/70 rounded-md w-3/4 mb-2 animate-pulse"></div>
                            @if($viewMode === 'list')
                                <div class="h-3.5 bg-zinc-200/70 rounded-md w-1/3 mb-2 animate-pulse"></div>
                            @endif
    ...
    {{-- Product List (hidden while loading) --}}
    <div wire:loading.remove wire:target="setCategory, setViewMode">
        <main class="max-w-xl mx-auto px-5 mt-4 {{ $viewMode === 'grid' ? 'grid grid-cols-2 gap-3' : 'flex flex-col gap-3' }}">
            @forelse($this->products as $index => $item)
                @php
                    $delay = $index < 20 ? $index * 50 : 0;
                @endphp
                <div
                    ...
                    class="bg-white rounded-2xl border border-zinc-100/80 shadow-sm flex group overflow-hidden relative transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 cursor-pointer animate-slide-up
                        {{ $viewMode === 'grid' ? 'flex-col h-full' : 'flex-row items-center gap-4 p-3' }}
                ...
                >
                    {{-- Image --}}
                    <div class="bg-zinc-100 overflow-hidden shrink-0 relative {{ $viewMode === 'grid' ? 'w-full aspect-[4/3]' : 'w-20 h-20 rounded-xl' }}">
                        @if($item['image'])
                            <img
                                src="{{ $item['image'] }}"
                                alt="{{ $item['name'] }}"
                        ...
                        <div class="absolute top-2.5 left-2.5 flex flex-col gap-2 z-20">
                            <button
                                onclick="event.stopPropagation(); navigator.share ? navigator.share({title:'{{ addslashes($item['name']) }}', url:window.location.origin+'/menu/{{ $item['id'] }}'}) : navigator.clipboard.writeText(window.location.origin+'/menu/{{ $item['id'] }}')"
                        ...
                        @if($viewMode === 'grid')
                            <div class="absolute top-2.5 right-2.5 bg-white/95 backdrop-blur-sm px-3 py-1.5 rounded-lg text-xs font-black shadow-md border border-white/50 z-20">
                        ...
                        <div class="mt-2.5" onclick="event.stopPropagation()">
                            <template x-if="showStepper">
                                <div class="flex items-center justify-between bg-zinc-900 rounded-xl p-1 shadow-md">
                                    ...
                                    <button @click.stop="updateQty(item.name, 1)" class="w-8 h-8 flex items-center justify-center text-zinc-900 bg-[var(--primary-color)] rounded-lg transition-all active:scale-90 hover:brightness-110">
                            ...
                            <template x-if="!showStepper">
                                <button
                                    ...
                                    @click.stop="item.has_variants ? openOption(item) : addToCart(item)"
