...
>
    ...
    >
        ...
        <template x-if="orderSuccess">
            <div class="flex-1 flex flex-col items-center justify-center p-8 text-center animate-fade-in-scale">
                <div class="w-20 h-20 rounded-full bg-emerald-50 flex items-center justify-center mb-6">
                    <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-emerald-500"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><path d="m9 11 3 3L22 4"/></svg>
                </div>
                <h3 class="text-xl font-black text-zinc-900 mb-2">Pesanan Terkirim! 🎉</h3>
                <p class="text-sm text-zinc-500 mb-2">Pesanan kamu sedang diproses oleh toko.</p>
                <div class="bg-zinc-50 rounded-xl px-5 py-3 border border-zinc-100 mb-6">
                    <span class="text-[10px] font-bold text-zinc-400 uppercase tracking-widest block mb-1">Invoice</span>
                    <span class="text-lg font-black text-zinc-900" x-text="orderSuccess.invoiceCode"></span>
                </div>
                <div class="bg-[var(--primary-color)]/10 rounded-xl px-5 py-3 border border-[var(--primary-color)]/20 mb-8">
                    <span class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest block mb-1">Total Bayar</span>
                    <span class="text-lg font-black text-zinc-900" x-text="orderSuccess.total"></span>
                </div>
                <button @click="closeCheckout" class="w-full max-w-xs bg-zinc-900 text-white py-4 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-zinc-800 transition-all active:scale-95">
                    Kembali ke Menu
                </button>
            </div>
        </template>
            ...
            <div class="flex-1 min-h-0 flex flex-col overflow-hidden">
                    ...
                    <template x-if="cart.length > 0">
                        <div>
                            <template x-for="item in cart" :key="item.cartName">
                                <div class="flex justify-between items-start mb-6 pb-6 border-b border-zinc-50 last:border-0 animate-slide-up">
                                    <div class="flex-1 pr-4">
                                        <h4 class="font-bold text-sm text-zinc-900 leading-tight" x-text="item.cartName"></h4>
                                        <p class="text-xs font-medium text-[var(--primary-color)] mt-1" x-text="formatPrice(item.price * item.qty)"></p>
                                    </div>
                ...
                <div class="shrink-0 p-6 bg-white border-t border-zinc-100 shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
                    ...
                    <div class="space-y-3 mb-4">
                        <div class="relative">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                            <input
                        ...
                        <div class="relative">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
                            <input
                    ...
                    >
                        ...
                        <span x-show="checkoutLoading" class="flex items-center gap-2" style="display: none;">
                            <svg class="animate-spin w-5 h-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg>
                            Memproses...
                        </span>
                    </button>
                </div>
