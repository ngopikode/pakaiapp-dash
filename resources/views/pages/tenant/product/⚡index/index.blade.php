<div class="container-fluid p-2" x-data="{ viewMode: 'grid' }">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-2">
        ...
        <div class="d-flex align-items-center gap-2">
            <!-- View Mode Toggles -->
            <div class="p-1 rounded-pill shadow-sm border d-none d-sm-flex">
                <button @click="viewMode = 'grid'"
                        :class="viewMode === 'grid' ? 'btn-primary shadow-sm' : 'text-muted'"
                        class="btn btn-sm rounded-pill px-3 transition-all">
                    <i class="bi bi-grid-fill"></i>
                </button>
                <button @click="viewMode = 'list'"
                        :class="viewMode === 'list' ? 'btn-primary shadow-sm' : 'text-muted'"
                        class="btn btn-sm rounded-pill px-3 transition-all">
            ...
            <button wire:click="$dispatch('openModal', { type: 'category', mode: 'create' })"
                    class="btn btn-primary border fw-bold px-3 py-2 rounded-pill shadow-sm flex-grow-1 flex-md-grow-0">
            ...
            <a href="{{ route('product.create') }}" wire:navigate
               class="btn btn-primary fw-bold px-3 py-2 rounded-pill shadow-sm flex-grow-1 flex-md-grow-0 d-flex justify-content-center align-items-center"
                ...
                <div class="accordion-item-op" wire:key="cat-{{ $category->id }}">
                    ...
                    <button wire:click="loadProducts({{ $category->id }})"
                        ...
                        <div class="d-flex align-items-center flex-grow-1 pe-3">
                            ...
                            <span class="badge text-muted border rounded-pill ms-auto"
                        ...
                        <div class="accordion-body-op p-3 p-md-4" wire:loading.remove
                            ...
                            <div class="d-flex justify-content-end mb-4 gap-2 border-bottom pb-3">
                                <button
                                    ...
                                    class="btn btn-sm btn-primary border fw-bold rounded-pill px-3 shadow-sm">
                                    <i class="bi bi-pencil me-1"></i> Edit Kategori
                                </button>
                                @if($category->products_count == 0)
                                    <button wire:click="deleteCategory({{ $category->id }})"
                                            wire:confirm="Hapus kategori ini secara permanen?"
                                            class="btn btn-sm btn-primary border border-danger text-danger fw-bold rounded-pill px-3 shadow-sm">
                                ...
                                <div class="row g-3">
                                    @forelse($loadedProducts as $product)
                                        <div class="col-6 col-md-4 col-xl-3" wire:key="prod-grid-{{ $product->id }}">
                                            <div
                                                class="card product-card h-100 overflow-hidden border border-1 {{ !$product->is_active ? 'opacity-75' : '' }}">

                                                <div class="ratio ratio-1x1 position-relative border-bottom">
                                                    @if($product->image)
                                                    ...
                                                    <div class="row g-1 mt-auto">
                                                        ...
                                                        <div class="col-4">
                                                            <button wire:click="toggleAvailability({{ $product->id }})"
                                                                    class="btn w-100 btn-action-mobile rounded-3 border {{ $product->is_active ? 'btn-light text-success' : 'btn-danger text-secondary border-danger' }}">
                            ...
                            <div x-show="viewMode === 'list'" x-cloak>
                                <div class="d-flex flex-column gap-2">
                                    @forelse($loadedProducts as $product)
                                        <div
                                            class="card product-card border border-1 flex-row align-items-center p-2 {{ !$product->is_active ? 'opacity-75' : '' }}"
                                            ...
                                            <div class="position-relative me-3 flex-shrink-0">
                                                ...
                                                @else
                                                    <div
                                                        class="border d-flex align-items-center justify-content-center text-muted shadow-sm"
                                            ...
                                            <div class="flex-grow-1 min-w-0 me-2">
                                                ...
                                                <span class="badge text-muted border rounded-pill"
