...
<div>
    ...
    <div class="bg-body border-bottom px-3 py-3 d-flex align-items-center justify-content-between sticky-top z-3">
        ...
        <button type="button" x-data="themeToggle" @click="toggleTheme()" class="btn btn-light rounded-circle shadow-sm" style="width: 40px; height: 40px;">
    ...
    <div class="container-fluid px-3 py-4" style="padding-bottom: 100px !important;">
        ...
        <!-- User Info Card -->
        <div class="card border-0 shadow-sm rounded-4 mb-4" style="background: linear-gradient(135deg, var(--brand-caramel, #B67332), var(--brand-espresso, #321E14));">
            <div class="card-body p-3 d-flex align-items-center gap-3">
                <div class="bg-white text-dark fw-bold rounded-circle d-flex align-items-center justify-content-center flex-shrink-0 shadow-sm" style="width: 50px; height: 50px; font-size: 1.2rem;">
                    {{ substr(Auth::user()->name ?? 'U', 0, 1) }}
                </div>
                <div class="text-white min-w-0">
                    <h5 class="mb-0 fw-bold text-truncate">{{ Auth::user()->name }}</h5>
                    <div class="small opacity-75 text-truncate">{{ Auth::user()->email }} &bull; {{ ucfirst(Auth::user()->role) }}</div>
                </div>
            </div>
        </div>
        ...
        @foreach($this->menuSections as $section)
            <div class="mb-4">
                <h6 class="text-secondary fw-bold ms-2 mb-2 text-uppercase" style="font-size: 0.75rem; letter-spacing: 0.5px;">{{ $section['title'] }}</h6>
            ...
            <div class="list-group list-group-flush">
                <style>
                    @media (display-mode: standalone) {
                        .pwa-install-item { display: none !important; }
                    }
                </style>
                <button type="button" onclick="if(window.installPwa) window.installPwa()" class="pwa-install-item list-group-item list-group-item-action d-flex align-items-center gap-3 py-3 px-3 text-success fw-bold">
                ...
                <button type="button" wire:click="logout" class="list-group-item list-group-item-action d-flex align-items-center gap-3 py-3 px-3 text-danger fw-bold border-top-0">
