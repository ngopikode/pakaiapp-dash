    ...
    <div class="d-flex flex-column mb-3 gap-3 pt-2">
        <div>
            <h6 class="page-greeting-label mb-1 text-secondary">
                Halo, {{ explode(' ', $user->name)[0] }} 👋
            </h6>
            <h2 class="page-store-name mb-0 d-flex align-items-center gap-2 fs-3">
                {{ $store->name ?? 'Setup Tokomu' }}
                @if($store)
                    <span class="badge bg-success bg-opacity-10 text-success rounded-pill px-2 py-1 fw-bold d-flex align-items-center gap-1 border border-success border-opacity-10" style="font-size: 0.6rem;">
        ...
        {{-- Mobile: Saldo & Action Buttons --}}
        <div class="d-flex flex-column w-100 gap-2 mt-1">
            {{-- Top Right Mini Wallet Info --}}
            <div class="d-flex align-items-center justify-content-between gap-3 py-2 px-3 rounded-4 border bg-body shadow-sm w-100" style="border-color: var(--bs-border-color-translucent) !important;">
                <div class="d-flex align-items-center gap-2">
                    ...
                    <div class="d-flex flex-column lh-1">
                        <span class="text-secondary small fw-bold" style="font-size: 0.6rem; letter-spacing: 1px; text-transform: uppercase;">Saldo Anda</span>
                        <div class="d-flex align-items-center gap-2 mt-1">
                            <span class="fw-bold text-body {{ $stats['wallet_balance'] < 3000 ? 'text-danger animate-pulse' : '' }}" style="font-family: var(--font-serif), sans-serif; font-size: 1rem;">
            ...
            <div class="d-flex flex-row gap-2 w-100">
                <button wire:click="exportLaporan"
                        class="btn btn-dashboard-header btn-outline flex-grow-1 py-2 px-3 rounded-4"
                        wire:loading.attr="disabled"
                        style="font-size: 0.85rem;">
                    <span wire:loading.remove wire:target="exportLaporan"><i class="bi bi-file-earmark-excel-fill text-success fs-6 me-1"></i> Rekap</span>
                    <span wire:loading wire:target="exportLaporan"><span class="spinner-border spinner-border-sm"></span></span>
                </button>
                <a href="{{ route('cashier') }}" wire:navigate.hover
                   class="btn btn-caramel-solid text-white flex-grow-1 py-2 px-3 d-flex align-items-center justify-content-center gap-1 rounded-4 shadow-sm"
                   style="font-size: 0.85rem;">
                    <i class="bi bi-cart-check-fill fs-6"></i> Kasir
                    @if($newOrderCount > 0)
                        <span class="badge rounded-circle ms-1 shadow-sm" style="padding: 0.35em 0.5em; background-color: #ffffff !important; color: #dc3545 !important; font-size: 0.7rem;">{{ $newOrderCount }}</span>
                    @endif
                </a>
            </div>
        </div>
    ...
    @else
        {{-- Modern Stats Row (Mobile Grid) --}}
        <div class="row g-2 mb-3">
            {{-- Omset Hari Ini --}}
            <div class="col-6">
                <div class="card h-100 dash-card position-relative overflow-hidden border bg-body p-2">
                    <div class="card-body p-2 position-relative z-1 d-flex flex-column justify-content-between">
                        <div class="d-flex justify-content-between align-items-start mb-2">
                            <span class="badge text-secondary border rounded-pill px-2 py-1 bg-body-tertiary" style="font-size: 0.6rem;">Hari Ini</span>
                        </div>
                        <div>
                            <div class="text-body fw-bold mb-1" style="font-size: 1.15rem; letter-spacing: -0.5px;">
                                Rp {{ number_format($stats['revenue_today'], 0, ',', '.') }}
                            </div>
                            <div class="d-flex flex-column gap-1">
                                <p class="text-secondary fw-bold mb-0 opacity-75" style="font-size: 0.6rem;">{{ $stats['orders_today'] }} Trx</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-6">
                <div class="card h-100 dash-card position-relative overflow-hidden border bg-body p-2">
                    <div class="card-body p-2 position-relative z-1 d-flex flex-column justify-content-between">
                        <div class="d-flex justify-content-between align-items-start mb-2">
                            <span class="badge text-secondary border rounded-pill px-2 py-1 bg-body-tertiary" style="font-size: 0.6rem;">Bulan Ini</span>
                        </div>
                        <div>
                            <div class="text-body fw-bold mb-1" style="font-size: 1.15rem; letter-spacing: -0.5px;">
                            ...
                            <div class="d-flex flex-column gap-1">
                                <p class="text-secondary fw-bold mb-0 opacity-75" style="font-size: 0.6rem;">
                                    Laba: {{ number_format($stats['profit_month'], 0, ',', '.') }}
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-12 mt-2">
                <div class="card dash-card position-relative overflow-hidden border bg-body p-2 {{ $stats['pending_orders'] > 0 ? 'border-danger border-2' : 'border-secondary border-opacity-25' }}"
            ...
            <div class="col-12">
                <div class="card dash-card bg-body border" style="border-color: var(--bs-border-color-translucent) !important;">
                    <div class="card-header bg-transparent border-0 pt-3 px-3">
                        <h6 class="fw-bold mb-0 text-body" style="font-family: var(--font-serif), sans-serif;"><i class="bi bi-graph-up text-primary me-2"></i>Trend Pendapatan (7 Hari)</h6>
                    </div>
                    <div class="card-body p-3 pt-1" wire:ignore>
                        <div id="revenueChart" style="min-height: 200px;"></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="d-flex flex-column gap-3 pb-5">
            {{-- Quick Actions --}}
            <div class="card dash-card p-2 bg-body border" style="border-color: var(--bs-border-color-translucent) !important;">
                <div class="card-body p-3 bg-body">
                    <h6 class="fw-bold mb-3 text-body" style="font-family: var(--font-serif), sans-serif;">Aksi Cepat</h6>
                    <a href="{{ route('product.create') }}" wire:navigate.hover class="quick-action-btn d-flex align-items-center gap-3 p-3 text-decoration-none text-body rounded-4 border bg-body-tertiary shadow-sm">
                        <div class="icon-wrapper bg-primary bg-opacity-10 text-primary rounded-circle p-2 flex-shrink-0 d-flex justify-content-center align-items-center" style="width: 40px; height: 40px;">
                            <i class="bi bi-plus-lg fs-5"></i>
                        </div>
                        <div>
                            <h6 class="fw-bold mb-0 small">Tambah Produk Baru</h6>
                            <small class="text-secondary d-block opacity-75" style="font-size: 0.65rem;">Perbarui katalog jualanmu</small>
                        </div>
                    </a>
                </div>
            </div>
                    ...
                    @if(count($topProducts) > 0)
                        <div class="d-flex flex-column gap-2">
                            @foreach($topProducts as $index => $item)
                                <div class="d-flex align-items-center justify-content-between p-2 rounded-3 border bg-body-tertiary">
                                    <div class="d-flex align-items-center gap-2">
                                    ...
                                    <span class="badge border shadow-sm rounded-pill bg-body text-secondary">{{ $item->total_sold }} Terjual</span>
                                </div>
                            @endforeach
                        </div>
                    @else
                        <div class="text-center py-3 rounded-4 bg-body-tertiary border">
                            <small class="text-secondary fw-bold">Belum ada data penjualan.</small>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="card dash-card p-2 bg-body border" style="border-color: var(--bs-border-color-translucent) !important;">
                <div class="card-header border-0 bg-transparent pt-3 px-3">
                    <h6 class="fw-bold mb-0 text-body" style="font-family: var(--font-serif), sans-serif;"><i class="bi bi-credit-card-2-front-fill me-2 text-primary"></i>Metode Pembayaran</h6>
                </div>
            ...
            <div class="card dash-card p-2 bg-body border" style="border-color: var(--bs-border-color-translucent) !important;">
                <div class="card-header border-0 bg-transparent pt-3 px-3">
                    <h6 class="fw-bold mb-0 text-body" style="font-family: var(--font-serif), sans-serif;"><i class="bi bi-shop-window me-2 text-warning"></i>Sumber Pesanan</h6>
                </div>
                <div class="card-body p-3 pt-1">
