    ...
    <div class="col-12 col-md-6 text-md-end">
        <div class="d-flex flex-wrap align-items-center justify-content-start justify-content-md-end gap-3">
            ...
            <div class="d-flex align-items-center justify-content-between gap-3 py-1.5 px-3 rounded-pill border bg-body shadow-sm" style="border-color: var(--bs-border-color-translucent) !important; font-size: 0.85rem;">
                <div class="d-flex align-items-center gap-2">
                    <div class="rounded-circle d-flex align-items-center justify-content-center bg-emerald-500 bg-opacity-10 text-emerald-600 flex-shrink-0" style="width: 28px; height: 28px;">
            ...
            <div class="d-flex align-items-center gap-2">
                <button wire:click="exportLaporan" class="btn btn-outline-secondary btn-sm py-2 px-3 rounded-3 d-flex align-items-center gap-1" wire:loading.attr="disabled" style="font-size: 0.8rem; font-weight: 600;">
                    <span wire:loading.remove wire:target="exportLaporan"><i class="ph-fill ph-file-csv text-success fs-6 me-1"></i> Export</span>
                    <span wire:loading wire:target="exportLaporan"><span class="spinner-border spinner-border-sm me-1"></span> Proses...</span>
                </button>
                <a href="{{ route('cashier') }}" wire:navigate.hover class="btn btn-success btn-sm py-2 px-3 rounded-3 d-flex align-items-center justify-content-center gap-1 text-white fw-bold" style="font-size: 0.8rem; font-weight: 750; background-color: #10B981; border: none;">
...
@if($newOrderCount > 0)
    <div class="notif-new-order p-3 mb-4 transition-all" style="position: relative; overflow: hidden; border-radius: 1rem;">
        ...
        <div class="d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between gap-3 position-relative z-1">
            ...
            <button wire:click="acknowledgeOrders" class="btn btn-emerald-solid flex-shrink-0 w-100 w-md-auto" type="button">
...
@else
    ...
    <div class="row g-2 g-md-3 mb-4">
        ...
        <div class="col-6 col-md-4">
            <div class="card h-100 rounded-3 shadow-sm border p-2 p-md-3">
                <div class="d-flex justify-content-between align-items-start mb-2 mb-md-3">
                    <div class="rounded-circle d-flex align-items-center justify-content-center bg-emerald-500 bg-opacity-10 text-emerald-600 flex-shrink-0" style="width: 32px; height: 32px; @media(min-width: 768px){ width: 40px; height: 40px; }">
                        <i class="ph-fill ph-wallet fs-6 fs-md-5" style="color: #10B981;"></i>
                    </div>
                    <span class="badge text-secondary border rounded-pill px-2 py-0.5 px-md-2.5 py-md-1 bg-body-tertiary" style="font-size: 0.65rem;">Hari Ini</span>
                </div>
                <div>
                    <h3 class="fw-bold text-body mb-1 mb-md-2 text-truncate" style="font-size: 1.1rem; @media(min-width: 768px){ font-size: 1.75rem; } letter-spacing: -0.5px;">
                        Rp {{ number_format($stats['revenue_today'], 0, ',', '.') }}
                    </h3>
                    <div class="d-flex flex-column gap-1">
                        <p class="text-secondary small fw-bold mb-0 opacity-75 text-truncate" style="font-size: 0.65rem;">
                            {{ $stats['orders_today'] }} Trx
                            @if($stats['revenue_trend_today'] != 0)
                                <span class="ms-0.5 px-1 rounded {{ $stats['revenue_trend_today'] > 0 ? 'bg-success text-white' : 'bg-danger text-white' }}" style="font-size: 0.6rem;">
        ...
        <div class="col-6 col-md-4">
            <div class="card h-100 rounded-3 shadow-sm border p-2 p-md-3">
                <div class="d-flex justify-content-between align-items-start mb-2 mb-md-3">
                    <div class="rounded-circle d-flex align-items-center justify-content-center bg-success bg-opacity-10 text-success flex-shrink-0" style="width: 32px; height: 32px; @media(min-width: 768px){ width: 40px; height: 40px; }">
                        <i class="ph-bold ph-trend-up fs-6 fs-md-5"></i>
                    </div>
                    <span class="badge text-secondary border rounded-pill px-2 py-0.5 px-md-2.5 py-md-1 bg-body-tertiary" style="font-size: 0.65rem;">Bulan Ini</span>
                </div>
                <div>
                    <h3 class="fw-bold text-body mb-1 mb-md-2 text-truncate" style="font-size: 1.1rem; @media(min-width: 768px){ font-size: 1.75rem; } letter-spacing: -0.5px;">
                    ...
                    <div class="d-flex flex-column gap-1">
                        <p class="text-secondary small fw-bold mb-0 opacity-75 text-truncate" style="font-size: 0.65rem;">
                            ...
                            @if($stats['revenue_trend_month'] != 0)
                                <span class="ms-0.5 px-1 rounded {{ $stats['revenue_trend_month'] > 0 ? 'bg-success text-white' : 'bg-danger text-white' }}" style="font-size: 0.6rem;">
        ...
        <div class="col-12 col-md-4">
            <div class="card h-100 rounded-3 shadow-sm border p-2 p-md-3 {{ $stats['pending_orders'] > 0 ? 'card-pending-alert' : '' }}">
                <div class="d-flex justify-content-between align-items-center mb-2 mb-md-3">
                    <div class="d-flex align-items-center gap-2">
                        <div class="rounded-circle d-flex align-items-center justify-content-center {{ $stats['pending_orders'] > 0 ? 'bg-danger bg-opacity-10 text-danger' : 'bg-secondary bg-opacity-10 text-secondary' }} flex-shrink-0" style="width: 32px; height: 32px; @media(min-width: 768px){ width: 40px; height: 40px; }">
                            <i class="bi bi-hourglass-split fs-6 fs-md-5"></i>
                        </div>
                        <span class="badge text-secondary border rounded-pill px-2 py-0.5 px-md-2.5 py-md-1 bg-body-tertiary" style="font-size: 0.65rem;">Antrean</span>
                    </div>
                    <div>
                        <h3 class="fw-bold {{ $stats['pending_orders'] > 0 ? 'text-danger' : 'text-body' }} mb-0" style="font-size: 1.25rem; @media(min-width: 768px){ font-size: 1.75rem; } letter-spacing: -0.5px;">
                            {{ $stats['pending_orders'] }} <span style="font-size: 0.9rem;" class="fw-medium">Pesanan</span>
                        </h3>
                    </div>
                </div>
                <div>
                    <p class="text-secondary small fw-bold mb-0 opacity-75" style="font-size: 0.7rem;">Menunggu Diproses di Kasir</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="row g-4 align-items-start">
        ...
        <div class="col-12 col-lg-8 d-flex flex-column gap-3">
            ...
            <div class="row g-3">
                ...
                <div class="col-12 col-md-6">
                    <div class="card h-100 rounded-3 shadow-sm border p-3">
                        <h6 class="fw-bold mb-3 text-body" style="font-size: 0.95rem;"><i class="ph-fill ph-storefront-window me-2 text-warning"></i>Sumber Pesanan (Bulan Ini)</h6>
