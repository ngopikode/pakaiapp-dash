...
## 2. Service Injection Guidelines
Ketika meng-inject Service atau Dependency ke Controller / Service lain:

1. **Constructor Promotion (Banyak Pemakaian)**:
   ```php
   public function __construct(protected readonly TargetService $targetService) {}
   ```

2. **Lazy Initialization (Sedikit Pemakaian)**:
   JANGAN pakai `app(TargetService::class)` inline berulang kali.
   ```php
...
## 3. Aturan Emas Pengembangan (Ponytail Mode)
