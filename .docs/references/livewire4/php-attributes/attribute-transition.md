...
`type`
`string`
The transition type name (e.g., `'forward'`, `'backward'`)

`skip`
`bool`
Set to `true` to disable transitions for this action
...
For more information about view transitions, see the [wire:transition documentation](../html-directives/wire-transition.md).
