...
For detailed information about computed properties, caching strategies, and advanced use cases, see the [Computed Properties documentation](../features/computed-properties.md).
## Reference
...
`bool`
`false`
Cache the value across requests for the same component instance

`$seconds`
`int`
`3600`
Duration in seconds to cache the value

`$cache`
`bool`
`false`
Cache the value across all component instances
...
`mixed`
`null`
Cache tags (requires a cache driver that supports tags)
