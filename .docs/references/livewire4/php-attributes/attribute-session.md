...
If you want state to be shareable via URL or bookmarkable, consider using the `#[Url]`[ attribute](../features/url.md) instead of `#[Session]`. URL parameters persist state in the address bar while session properties keep the URL clean.
