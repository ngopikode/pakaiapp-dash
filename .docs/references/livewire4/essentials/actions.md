...
For most cases, using `data-loading` selectors is simpler and more flexible than `wire:loading`. [Learn more about loading states →](../features/loading-states.md)
...
Learn more by reading the [documentation for using Alpine inside Livewire](../features/alpine.md).
...
For more information, visit the `wire:confirm`[ documentation page](../html-directives/wire-confirm.md).
...
To make this pairing even more powerful, Livewire exposes a magic `$wire` object to Alpine that can be treated as a JavaScript representation of your PHP component. In addition to `$wire`[accessing and mutating public properties via ](/docs/4.x/properties#accessing-properties-from-javascript), you can call actions. When an action is invoked on the `$wire` object, the corresponding PHP method will be invoked on your backend Livewire component:
...
For actions primarily consumed by JavaScript, consider using the `#[Json]`[ attribute](../php-attributes/attribute-json.md). It returns data via promise resolution/rejection, automatically handles validation errors with promise rejection, and skips re-rendering for better performance.
...
[Javascript reference](/docs/4.x/javascript#the-wire-object-1)

> [!NOTE]
> > For more information about magic actions, see
