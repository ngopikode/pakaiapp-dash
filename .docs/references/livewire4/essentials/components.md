...
`--type=sfc|mfc|class`
Set the component type explicitly

`--emoji=true|false`
Override the config emoji setting for this command

`--test`
Include a Pest test file

`--js`
Include a JavaScript file (multi-file components only)

`--css`
Include CSS files (multi-file components only)
...
Multi-file
`resources/views/components/post/⚡create/create.php`
`post.create`
...
Single-file (namespaced)
`resources/views/pages/post/⚡create.blade.php`
`pages::post.create`

Multi-file (namespaced)
`resources/views/pages/post/⚡create/create.php`
`pages::post.create`
...
You can think of the `mount()` method as a class constructor. It runs when the component initializes, but not on subsequent requests within a page's session. You can learn more about `mount()` and other helpful lifecycle hooks within the [lifecycle documentation](lifecycle-hooks.md).
...
For complete information about page components, including layouts, titles, and advanced routing, see the [Pages documentation](pages.md).
...
For complete information about properties, including persistence behavior and advanced features, see the [properties documentation](properties.md).
### Computed properties
