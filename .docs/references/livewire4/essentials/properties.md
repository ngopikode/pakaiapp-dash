...
This is just scratching the surface of `wire:model`. For deeper information on data binding, check out our [documentation on forms](forms.md).
...
Wireables are simple and easy to use for most applications, so we'll explore them below. If you're an advanced user or package author wanting more flexibility, [Synthesizers are the way to go](../advanced/synthesizers.md).
#### Wireables
...
As mentioned earlier, if you want to support types more globally and powerfully, Livewire offers Synthesizers, its advanced internal mechanism for handling different property types. [Learn more about Synthesizers](../advanced/synthesizers.md).
...
For more information on locking properties, [consult the Locked attribute documentation](../php-attributes/attribute-locked.md).
...
To ensure the integrity of Eloquent queries, we recommend that you use [computed properties](../features/computed-properties.md) instead of properties.
...
For more information, [visit the computed properties documentation](../features/computed-properties.md).
## See also
...
* [Locked Attribute](../php-attributes/attribute-locked.md) — Prevent properties from being manipulated client-side
