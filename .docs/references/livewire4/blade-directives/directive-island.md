...
`?string`
`null`
A unique name for targeting the island with `wire:island`

`$lazy`
`bool`
`false`
Defer rendering until the island is visible in the viewport

`$defer`
`bool`
`false`
Load immediately after page load instead of waiting for viewport visibility
