...
`.remove`
Show element by default, hide when dirty

`.class="class-name"`
Add a CSS class when dirty
...
`$dirty('property')`
Returns `true` if the specified property has unsaved changes

`$dirty(['title', 'description'])`
Returns `true` if any of the specified properties have unsaved changes
