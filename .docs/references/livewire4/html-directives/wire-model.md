...
For a more complete documentation on using `wire:model` in the context of HTML forms, visit the [Livewire forms documentation page](../essentials/forms.md).
## See also
...
`.lazy`
Update on change and send network request (v3 compatible)

`.debounce.Xms`
Debounce updates (use with `.live`)

`.throttle.Xms`
Throttle updates (use with `.live`)

`.number`
Cast value to `int` on the server

`.boolean`
Cast value to `bool` on the server

`.fill`
Use initial value from HTML `value` attribute

`.deep`
Also listen to events from child elements
