...
`.once`
Only fire the action on the first intersection

`.half`
Trigger when half of the element is visible

`.full`
Trigger when the entire element is visible

`.threshold.[0-100]`
Trigger at a custom visibility threshold percentage

`.margin.[value]`
Add margin around the viewport (e.g., `.margin.200px`, `.margin.10%`)
