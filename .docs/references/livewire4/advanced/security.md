...
Another way to prevent properties from being set to unwanted values is to use `#[Locked]`[the  attribute](../php-attributes/attribute-locked.md). Locking properties is done by applying the `#[Locked]` attribute. Now if users attempt to tamper with this value an error will be thrown.
