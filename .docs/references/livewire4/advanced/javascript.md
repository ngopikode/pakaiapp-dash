...
Here's an example of using a simple `setInterval` to refresh the component every 2 seconds (You could easily do this with `wire:poll`[](../html-directives/wire-poll.md), but it's a simple way to demonstrate the point):
...
You can learn more about these objects in `$wire`[the  documentation](#the-wire-object).
    

### Interacting with events

In addition to dispatching and listening for events from individual components in PHP, the global `Livewire` object allows you to interact with [Livewire's event system](../essentials/events.md) from anywhere in your application:
...
* [Styles](../features/styles.md) — Add scoped CSS to your components

* [Alpine](../features/alpine.md) — Use Alpine for client-side interactivity

* [Actions](../essentials/actions.md) — Create JavaScript actions in components

* [Properties](../essentials/properties.md) — Access properties from JavaScript with $wire

* [Events](../essentials/events.md) — Dispatch and listen for events in JavaScript
