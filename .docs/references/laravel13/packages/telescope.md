...
Finally, you should also prevent the Telescope package from being [auto-discovered](../digging-deeper/packages.md#package-discovery) by adding the following to your `composer.json` file:
...
Without pruning, the `telescope_entries` table can accumulate records very quickly. To mitigate this, you should [schedule](../digging-deeper/scheduling.md) the `telescope:prune` Artisan command to run daily:
...
The batch watcher records information about queued [batches](../digging-deeper/queues.md#job-batching), including the job and connection information.
...
The event watcher records the payload, listeners, and broadcast data for any [events](../digging-deeper/events.md) dispatched by your application. The Laravel framework's internal events are ignored by the Event watcher.
...
The gate watcher records the data and result of [gate and policy](../security/authorization.md) checks by your application. If you would like to exclude certain abilities from being recorded by the watcher, you may specify those in the `ignore_abilities` option in your `config/telescope.php` file:
...
The HTTP client watcher records outgoing [HTTP client requests](../digging-deeper/http-client.md) made by your application.

### [Job Watcher](#job-watcher)

The job watcher records the data and status of any [jobs](../digging-deeper/queues.md) dispatched by your application.

### [Log Watcher](#log-watcher)

The log watcher records the [log data](../the-basics/logging.md) for any logs written by your application.
...
The mail watcher allows you to view an in-browser preview of [emails](../digging-deeper/mail.md) sent by your application along with their associated data. You may also download the email as an `.eml` file.

### [Model Watcher](#model-watcher)

The model watcher records model changes whenever an Eloquent [model event](../eloquent-orm/eloquent.md#events) is dispatched. You may specify which model events should be recorded via the watcher's `events` option:
...
The notification watcher records all [notifications](../digging-deeper/notifications.md) sent by your application. If the notification triggers an email and you have the mail watcher enabled, the email will also be available for preview on the mail watcher screen.
...
The Redis watcher records all [Redis](../database/redis.md) commands executed by your application. If you are using Redis for caching, cache commands will also be recorded by the Redis watcher.
...
The schedule watcher records the command and output of any [scheduled tasks](../digging-deeper/scheduling.md) run by your application.

### [View Watcher](#view-watcher)

The view watcher records the [view](../the-basics/views.md) name, path, data, and "composers" used when rendering views.
