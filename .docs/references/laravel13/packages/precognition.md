...
When Laravel receives a "precognitive request", it will execute all of the route's middleware and resolve the route's controller dependencies, including validating [form requests](../the-basics/validation.md#form-request-validation) - but it will not actually execute the route's controller method.
...
First, to enable Precognition for a route, the `HandlePrecognitiveRequests` middleware should be added to the route definition. You should also create a [form request](../the-basics/validation.md#form-request-validation) to house the route's validation rules:
