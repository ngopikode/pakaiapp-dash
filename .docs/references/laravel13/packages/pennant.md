...
Feature classes are resolved via the [container](../architecture-concepts/container.md), so you may inject dependencies into the feature class's constructor when needed.
...
If your feature is returning a `Lottery` instance, there are a handful of useful [testing helpers available](../digging-deeper/helpers.md#testing-lotteries).
...
Once your driver has been implemented, you are ready to register it with Laravel. To add additional drivers to Pennant, you may use the `extend` method provided by the `Feature` facade. You should call the `extend` method from the `boot` method of one of your application's [service provider](../architecture-concepts/providers.md):
