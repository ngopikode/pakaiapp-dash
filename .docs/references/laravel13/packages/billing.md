...
Be sure to add a [date cast](../eloquent-orm/eloquent-mutators.md#date-casting) for the `trial_ends_at` attribute within your billable model's class definition.
...
Since Stripe webhooks need to bypass Laravel's [CSRF protection](../the-basics/csrf.md), you should ensure that Laravel does not attempt to validate the CSRF token for incoming Stripe webhooks. To accomplish this, you should exclude `stripe/*` from CSRF protection in your application's `bootstrap/app.php` file:
...
Both events contain the full payload of the Stripe webhook. For example, if you wish to handle the `invoice.payment_succeeded` webhook, you may register a [listener](../digging-deeper/events.md#defining-listeners) that will handle the event:
