...
For convenience, you may wish to create a [middleware](../the-basics/middleware.md) which determines if the incoming request is from a subscribed user. Once this middleware has been defined, you may easily assign it to a route to prevent users that are not subscribed from accessing the route:
...
Cashier includes a `paddle-button` [Blade component](../the-basics/blade.md#components). You may pass the checkout session to this component as a "prop". Then, when this button is clicked, Paddle's checkout widget will be displayed:
...
The `subscribed` method also makes a great candidate for a [route middleware](../the-basics/middleware.md), allowing you to filter access to routes and controllers based on the user's subscription status:
...
Make sure you protect incoming requests with Cashier's included [webhook signature verification](cashier-paddle.md#verifying-webhook-signatures) middleware.

#### [Webhooks and CSRF Protection](#webhooks-csrf-protection)

Since Paddle webhooks need to bypass Laravel's [CSRF protection](../the-basics/csrf.md), you should ensure that Laravel does not attempt to verify the CSRF token for incoming Paddle webhooks. To accomplish this, you should exclude `paddle/*` from CSRF protection in your application's `bootstrap/app.php` file:
...
Both events contain the full payload of the Paddle webhook. For example, if you wish to handle the `transaction.billed` webhook, you may register a [listener](../digging-deeper/events.md#defining-listeners) that will handle the event:
...
For automated tests, including those executed within a CI environment, you may use [Laravel's HTTP Client](../digging-deeper/http-client.md#testing) to fake HTTP calls made to Paddle. Although this does not test the actual responses from Paddle, it does provide a way to test your application without actually calling Paddle's API.