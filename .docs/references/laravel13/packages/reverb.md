...
[Laravel Reverb](https://github.com/laravel/reverb) brings blazing-fast and scalable real-time WebSocket communication directly to your Laravel application, and provides seamless integration with Laravel's existing suite of [event broadcasting tools](../digging-deeper/broadcasting.md).
...
Next, you should have a dedicated, central Redis server to which all of the Reverb servers will communicate. Reverb will use the [default Redis connection configured for your application](../database/redis.md#configuration) to publish messages to all of your Reverb servers.
...
Reverb dispatches internal events during the lifecycle of a connection and message handling. You may [listen for these events](../digging-deeper/events.md) to perform actions when connections are managed or messages are exchanged.
