...
The Laravel `Schema` [facade](../architecture-concepts/facades.md) provides database agnostic support for creating and manipulating tables across all of Laravel's supported database systems. Typically, migrations will use this facade to create and modify database tables and columns.

## [Generating Migrations](#generating-migrations)

You may use the `make:migration` [Artisan command](../digging-deeper/artisan.md) to generate a database migration. The new migration will be placed in your `database/migrations` directory. Each migration filename contains a timestamp that allows Laravel to determine the order of the migrations:
...
Migration stubs may be customized using [stub publishing](../digging-deeper/artisan.md#stub-customization).
...
This method is intended to be used when defining the columns necessary for a polymorphic [Eloquent relationship](../eloquent-orm/eloquent-relationships.md). In the following example, `taggable_type` and `taggable_id` columns would be created:
...
The `rememberToken` method creates a nullable, `VARCHAR(100)` equivalent column that is intended to store the current "remember me" [authentication token](../security/authentication.md#remembering-users):
...
This method is intended to be used when defining the columns necessary for a polymorphic [Eloquent relationship](../eloquent-orm/eloquent-relationships.md) that use ULID identifiers. In the following example, `taggable_type` and `taggable_id` columns would be created:
...
This method is intended to be used when defining the columns necessary for a [polymorphic Eloquent relationship](../eloquent-orm/eloquent-relationships.md#polymorphic-relationships) that use UUID identifiers. In the following example, `taggable_type` and `taggable_id` columns would be created:
...
For convenience, each migration operation will dispatch an [event](../digging-deeper/events.md). All of the following events extend the base `Illuminate\Database\Events\MigrationEvent` class:
