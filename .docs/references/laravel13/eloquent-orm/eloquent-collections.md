...
All Eloquent methods that return more than one model result will return instances of the `Illuminate\Database\Eloquent\Collection` class, including results retrieved via the `get` method or accessed via a relationship. The Eloquent collection object extends Laravel's [base collection](../digging-deeper/collections.md), so it naturally inherits dozens of methods used to fluently work with the underlying array of Eloquent models. Be sure to review the Laravel collection documentation to learn all about these helpful methods!
...
While most Eloquent collection methods return a new instance of an Eloquent collection, the `collapse`, `flatten`, `flip`, `keys`, `pluck`, and `zip` methods return a [base collection](../digging-deeper/collections.md) instance. Likewise, if a `map` operation returns a collection that does not contain any Eloquent models, it will be converted to a base collection instance.

## [Available Methods](#available-methods)

All Eloquent collections extend the base [Laravel collection](../digging-deeper/collections.md#available-methods) object; therefore, they inherit all of the powerful methods provided by the base collection class.
...
The `mergeVisible` method [makes additional attributes visible](eloquent-serialization.md#hiding-attributes-from-json) while retaining existing visible attributes:
...
The `mergeHidden` method [hides additional attributes](eloquent-serialization.md#hiding-attributes-from-json) while retaining existing hidden attributes:
...
The `setAppends` method temporarily overrides all of the [appended attributes](eloquent-serialization.md#appending-values-to-json) on each model in the collection:
...
The `withoutAppends` method temporarily removes all of the [appended attributes](eloquent-serialization.md#appending-values-to-json) on each model in the collection:
