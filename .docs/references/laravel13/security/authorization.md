...
All policies are resolved via the Laravel [service container](../architecture-concepts/container.md), allowing you to type-hint any needed dependencies in the policy's constructor to have them automatically injected.
...
Laravel includes a middleware that can authorize actions before the incoming request even reaches your routes or controllers. By default, the `Illuminate\Auth\Middleware\Authorize` middleware may be attached to a route using the `can` [middleware alias](../the-basics/middleware.md#middleware-aliases), which is automatically registered by Laravel. Let's explore an example of using the `can` middleware to authorize that a user can update a post:
...
If you are using [controller middleware attributes](../the-basics/controllers.md#middleware-attributes), you may apply the `can` middleware via the `Authorize` attribute:
...
However, if you are using one of Laravel's Inertia-based [starter kits](../getting-started/starter-kits.md), your application already contains a `HandleInertiaRequests` middleware. Within this middleware's `share` method, you may return shared data that will be provided to all Inertia pages in your application. This shared data can serve as a convenient location to define authorization information for the user:
