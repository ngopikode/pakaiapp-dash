...
In addition, you may copy this file to another language directory to translate the messages for your application's language. To learn more about Laravel localization, check out the complete [localization documentation](../digging-deeper/localization.md).
...
You may type-hint any dependencies you require within the `rules` method's signature. They will automatically be resolved via the Laravel [service container](../architecture-concepts/container.md).
...
Need to add real-time form request validation to your Inertia powered Laravel frontend? Check out [Laravel Precognition](../packages/precognition.md).
...
You may type-hint any dependencies you need within the `authorize` method's signature. They will automatically be resolved via the Laravel [service container](../architecture-concepts/container.md).
...
If you do not want to use the `validate` method on the request, you may create a validator instance manually using the `Validator` [facade](../architecture-concepts/facades.md). The `make` method on the facade generates a new validator instance:
...
If you would like to retrieve the validated data as a [collection](../digging-deeper/collections.md) instance, you may call the `collect` method:
...
#### [after\_or\_equal:*date*](#rule-after-or-equal)
...
#### [before\_or\_equal:*date*](#rule-before-or-equal)
...
#### [doesnt\_contain:*foo*,*bar*,...](#rule-doesnt-contain)
...
The field under validation must match the authenticated user's password. You may specify an [authentication guard](../security/authentication.md) using the rule's first parameter:
...
#### [date\_equals:*date*](#rule-date-equals)

The field under validation must be equal to the given date. The dates will be passed into the PHP `strtotime` function in order to be converted into a valid `DateTime` instance.

#### [date\_format:*format*,...](#rule-date-format)
...
#### [digits\_between:*min*,*max*](#rule-digits-between)
...
Available constraints are: *min\_width*, *max\_width*, *min\_height*, *max\_height*, *width*, *height*, *ratio*, *min\_ratio*, *max\_ratio*.
...
The *min\_ratio* and *max\_ratio* constraints may be used to define a range of acceptable aspect ratios:
...
#### [doesnt\_start\_with:*foo*,*bar*,...](#rule-doesnt-start-with)

The field under validation must not start with one of the given values.

#### [doesnt\_end\_with:*foo*,*bar*,...](#rule-doesnt-end-with)
...
#### [encoding:*encoding\_type*](#rule-encoding)
...
#### [ends\_with:*foo*,*bar*,...](#rule-ends-with)
...
#### [exclude\_if:*anotherfield*,*value*](#rule-exclude-if)
...
#### [exclude\_unless:*anotherfield*,*value*](#rule-exclude-unless)
...
#### [exclude\_with:*anotherfield*](#rule-exclude-with)
...
#### [exclude\_without:*anotherfield*](#rule-exclude-without)
...
#### [in\_array:*anotherfield*.\*](#rule-in-array)

The field under validation must exist in *anotherfield*'s values.

#### [in\_array\_keys:*value*.\*](#rule-in-array-keys)
...
#### [max\_digits:*value*](#rule-max-digits)
...
#### [min\_digits:*value*](#rule-min-digits)

The integer under validation must have a minimum length of *value*.

#### [multiple\_of:*value*](#rule-multiple-of)
...
#### [not\_regex:*pattern*](#rule-not-regex)
...
#### [present\_if:*anotherfield*,*value*,...](#rule-present-if)

The field under validation must be present if the *anotherfield* field is equal to any *value*.

#### [present\_unless:*anotherfield*,*value*](#rule-present-unless)

The field under validation must be present unless the *anotherfield* field is equal to any *value*.

#### [present\_with:*foo*,*bar*,...](#rule-present-with)

The field under validation must be present *only if* any of the other specified fields are present.

#### [present\_with\_all:*foo*,*bar*,...](#rule-present-with-all)
...
#### [prohibited\_if:*anotherfield*,*value*,...](#rule-prohibited-if)
...
#### [required\_if:*anotherfield*,*value*,...](#rule-required-if)
...
#### [required\_with:*foo*,*bar*,...](#rule-required-with)
...
#### [required\_with\_all:*foo*,*bar*,...](#rule-required-with-all)
...
#### [required\_without:*foo*,*bar*,...](#rule-required-without)

The field under validation must be present and not empty *only when* any of the other specified fields are empty or not present.

#### [required\_without\_all:*foo*,*bar*,...](#rule-required-without-all)
...
#### [required\_array\_keys:*foo*,*bar*,...](#rule-required-array-keys)
...
#### [starts\_with:*foo*,*bar*,...](#rule-starts-with)
...
Instead of providing a literal error message to the `$fail` closure, you may also provide a [translation string key](../digging-deeper/localization.md) and instruct Laravel to translate the error message:
