...
You may write information to the logs using the `Log` [facade](../architecture-concepts/facades.md). As previously mentioned, the logger provides the eight logging levels defined in the [RFC 5424 specification](https://tools.ietf.org/html/rfc5424): **emergency**, **alert**, **critical**, **error**, **warning**, **notice**, **info** and **debug**:
...
If you need to share log context while processing queued jobs, you may utilize [job middleware](../digging-deeper/queues.md#job-middleware).
...
All of your "tap" classes are resolved by the [service container](../architecture-concepts/container.md), so any constructor dependencies they require will automatically be injected.
