...
If you are using [Laravel Sail](../packages/sail.md) as your local development environment, remember to use the `sail` command line to invoke Artisan commands. Sail will execute your Artisan commands within your application's Docker containers:
...
Let's take a look at an example command. Note that we are able to request any dependencies we need via the command's `handle` method. The Laravel [service container](../architecture-concepts/container.md) will automatically inject all dependencies that are type-hinted in this method's signature:
...
The comprehensive [Laravel Prompts](../packages/prompts.md) documentation includes additional information on the available prompts and their usage.
...
[Laravel Prompts](../packages/prompts.md) is a PHP package for adding beautiful and user-friendly forms to your command-line applications, with browser-like features including placeholder text and validation.
...
When Artisan boots, all the commands in your application will be resolved by the [service container](../architecture-concepts/container.md) and registered with Artisan.
...
The `dev` Artisan command starts all of the processes needed for local development in a single terminal window. By default, it concurrently runs the PHP development server, a queue worker, log tailing via [Pail](../the-basics/logging.md#tailing-log-messages-using-pail), and Vite asset compilation:
