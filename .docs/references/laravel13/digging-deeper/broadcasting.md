...
Once you have enabled event broadcasting, you're ready to learn more about [defining broadcast events](#defining-broadcast-events) and [listening for events](#listening-for-events). If you're using Laravel's React, Vue, or Svelte [starter kits](../getting-started/starter-kits.md), you may listen for events using Echo's [useEcho hook](#using-react-or-vue).
...
When running the `install:broadcasting` command, you will be prompted to install [Laravel Reverb](../packages/reverb.md). Of course, you may also install Reverb manually using the Composer package manager:
...
You can find detailed Reverb installation and usage instructions in the [Reverb documentation](../packages/reverb.md).
...
To learn more about compiling your application's JavaScript assets, please consult the documentation on [Vite](../the-basics/vite.md).
...
Just like HTTP routes, channel routes may also take advantage of implicit and explicit [route model binding](../the-basics/routing.md#route-model-binding). For example, instead of receiving a string or numeric order ID, you may request an actual `Order` model instance:
...
Unlike HTTP route model binding, channel model binding does not support automatic [implicit model binding scoping](../the-basics/routing.md#implicit-model-binding-scoping). However, this is rarely a problem because most channels can be scoped based on a single model's unique, primary key.
...
Like many other classes in Laravel, channel classes will automatically be resolved by the [service container](../architecture-concepts/container.md). So, you may type-hint any dependencies required by your channel in its constructor.
...
Events that implement the `ShouldRescue` interface automatically utilize Laravel's [rescue helper function](helpers.md#method-rescue) during broadcast attempts. This helper catches any exceptions, reports them to your application's exception handler for logging, and allows the application to continue executing normally without interrupting the user's workflow:
...
It is common to broadcast events when your application's [Eloquent models](../eloquent-orm/eloquent.md) are created, updated, or deleted. Of course, this can easily be accomplished by manually [defining custom events for Eloquent model state changes](../eloquent-orm/eloquent.md#events) and marking those events with the `ShouldBroadcast` interface.
