...
Laravel Horizon is a beautiful dashboard and configuration system for your Redis powered queues. Check out the full [Horizon documentation](../packages/horizon.md) for more information.
...
In order to use the `database` queue driver, you will need a database table to hold the jobs. Typically, this is included in Laravel's default `0001_01_01_000002_create_jobs_table.php` [database migration](../database/migrations.md); however, if your application does not contain this migration, you may use the `make:queue-table` Artisan command to create it:
...
Job stubs may be customized using [stub publishing](artisan.md#stub-customization).
...
In this example, note that we were able to pass an [Eloquent model](../eloquent-orm/eloquent.md) directly into the queued job's constructor. Because of the `Queueable` trait that the job is using, Eloquent models and their loaded relationships will be gracefully serialized and unserialized when the job is processing.
...
The `handle` method is invoked when the job is processed by the queue. Note that we are able to type-hint dependencies on the `handle` method of the job. The Laravel [service container](../architecture-concepts/container.md) automatically injects these dependencies.

If you would like to take total control over how the container injects dependencies into the `handle` method, you may use the container's `bindMethod` method. The `bindMethod` method accepts a callback which receives the job and the container. Within the callback, you are free to invoke the `handle` method however you wish. Typically, you should call this method from the `boot` method of your `App\Providers\AppServiceProvider` [service provider](../architecture-concepts/providers.md):
...
Unique jobs require a cache driver that supports [locks](cache.md#atomic-locks). Currently, the `memcached`, `redis`, `dynamodb`, `database`, `file`, and `array` cache drivers support atomic locks.
...
Laravel allows you to ensure the privacy and integrity of a job's data via [encryption](../security/encryption.md). To get started, simply add the `ShouldBeEncrypted` interface to the job class. Once this interface has been added to the class, Laravel will automatically encrypt your job before pushing it onto a queue:
...
As you can see, like [route middleware](../the-basics/middleware.md), job middleware receive the job being processed and a callback that should be invoked to continue processing the job.
...
Job middleware can also be assigned to [queueable event listeners](events.md#queued-event-listeners), [mailables](mail.md#queueing-mail), and [notifications](notifications.md#queueing-notifications).

### [Rate Limiting](#rate-limiting)

Although we just demonstrated how to write your own rate limiting job middleware, Laravel actually includes a rate limiting middleware that you may utilize to rate limit jobs. Like [route rate limiters](../the-basics/routing.md#defining-rate-limiters), job rate limiters are defined using the `RateLimiter` facade's `for` method.
...
The `WithoutOverlapping` middleware requires a cache driver that supports [locks](cache.md#atomic-locks). Currently, the `memcached`, `redis`, `dynamodb`, `database`, `file`, and `array` cache drivers support atomic locks.
...
You may also define a `Tries` attribute or `retryUntil` method on your [queued event listeners](events.md#queued-event-listeners) and [queued notifications](notifications.md#queueing-notifications).
...
To define the message group for a [queued event listener](events.md#queued-event-listeners), define a `messageGroup` method on the listener. You may also optionally define a `deduplicationId` method:
...
When sending a [mail message](mail.md) that is going to be queued on a FIFO queue, you should invoke the `onGroup` method and optionally the `withDeduplicator` method when sending the notification:
...
When sending a [notification](notifications.md) that is going to be queued on a FIFO queue, you should invoke the `onGroup` method and optionally the `withDeduplicator` method when sending the notification:
...
Some tools such as [Laravel Horizon](../packages/horizon.md) and [Laravel Telescope](../packages/telescope.md) may provide more user-friendly debug information for batches if batches are named. To assign an arbitrary name to a batch, you may call the `name` method while defining the batch:
...
Without pruning, the `job_batches` table can accumulate records very quickly. To mitigate this, you should [schedule](scheduling.md) the `queue:prune-batches` Artisan command to run daily:
...
While your application is in [maintenance mode](../getting-started/configuration.md#maintenance-mode), no queued jobs will be handled. The jobs will continue to be handled as normal once the application is out of maintenance mode.
...
Daemon queue workers do not "reboot" the framework before processing each job. Therefore, you should release any heavy resources after each job completes. For example, if you are doing [image manipulation](images.md) with the [GD library](https://www.php.net/manual/en/book.image.php), you should free the memory with `imagedestroy` when you are done processing the image.
...
When using [Horizon](../packages/horizon.md), you should use the `horizon:forget` command to delete a failed job instead of the `queue:forget` command.
...
When using [Horizon](../packages/horizon.md), you should use the `horizon:clear` command to clear jobs from the queue instead of the `queue:clear` command.
...
To test job chains, you will need to utilize the `Bus` facade's faking capabilities. The `Bus` facade's `assertChained` method may be used to assert that a [chain of jobs](queues.md#job-chaining) was dispatched. The `assertChained` method accepts an array of chained jobs as its first argument:
...
The `Bus` facade's `assertBatched` method may be used to assert that a [batch of jobs](queues.md#job-batching) was dispatched. The closure given to the `assertBatched` method receives an instance of `Illuminate\Bus\PendingBatch`, which may be used to inspect the jobs within the batch:
...
Using the `before` and `after` methods on the `Queue` [facade](../architecture-concepts/facades.md), you may specify callbacks to be executed before or after a queued job is processed. These callbacks are a great opportunity to perform additional logging or increment statistics for a dashboard. Typically, you should call these methods from the `boot` method of a [service provider](../architecture-concepts/providers.md). For example, we may use the `AppServiceProvider` that is included with Laravel:
...
Using the `looping` method on the `Queue` [facade](../architecture-concepts/facades.md), you may specify callbacks that execute before the worker attempts to fetch a job from a queue. For example, you might register a closure to rollback any transactions that were left open by a previously failed job:
