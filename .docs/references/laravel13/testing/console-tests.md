...
In addition to simplifying HTTP testing, Laravel provides a simple API for testing your application's [custom console commands](../digging-deeper/artisan.md).
...
If you are utilizing the `search` or `multisearch` functions provided by [Laravel Prompts](../packages/prompts.md), you may use the `expectsSearch` assertion to mock the user's input, search results, and selection:
